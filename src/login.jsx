...
export default class Login extends React.Component {
  ...
  handleKeyDown(event) {
    if (event.target.name === 'employeenumber' && event.key === 'Enter' && !(this.state.employeenumber.length < 1 || this.state.submitting)) {
      ...
      if (this.login.trim().length > 1) this.login();
              ...
              <div className="card-body">
                ...
                <div className="form-group mb-0">
                  ...
                  <div className="d-flex flex-direction-row mt-2" >
                    <button type="button" className="ml-auto btn btn-primary" onClick={this.login} disabled={this.state.employeenumber.length < 1 || this.state.submitting || this.state.login.trim().length < 4}>Log in</button>
                  </div>
                </div>
              </div>
            </div>
